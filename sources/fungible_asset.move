module aptos_asset::fungible_asset{
    ...
    //Importing functionalities for fungible assets,including self-reference,minting,transferingburning,metadata,and fungible asset utilities.
    ...
    struct ManagedFungibleAsset has key{
        ...
        burn_ref:BurnRef,//Referencev for burning assets
    }

    fun init_module(admin:&signer){
        //Create a named object for the asset using the admin signer and the asset sumbol
        let constructor_ref = &object::create_named_object(admin,ASSET_SYMBOL);

        //Create the primary store for fungible asset with the specified metadata
        primary_fungible_store::create_primary_store_enable_fungible_asset(
            constructor_ref,
    ...
    public entry fun mint(admin: &signer,to: address,amount: u64)acquires ManagedFungibleAsset{
        ...
        let to_wallet = primary_fungible_store::ensure_primary_store_exits(to,asset);
    ...
    ///Burn fungible assets as the owner metadata object.
    public entry fun burn(admin: &signer,from: address,amount: u64)acquires ManagedFungibleAsset{
        let asset = get_metadata();
        let burn_ref = &authorized_boorow_refs(admin,asset).burn_ref;
        let from_wallet = primary_fungible_store::primary_store(from,asset);
